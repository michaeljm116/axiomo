...
Sys_Bvh :: struct {
    world: ^ecs.World, // ECS world reference
...
set_children :: proc "c" (bvhNodePtr: rawptr, childPtr: ^^rawptr, numChildren: u32, userPtr: rawptr) {
    ...
    for i in 0..<2 {
        child_ptr := mem.ptr_offset(childPtr, i)
        node.children[i] = cast(BvhNode)child_ptr^
    }
}
...
bvh_system_build :: proc(system: ^Sys_Bvh) {
    ...
    for prim_comp, i in system.primitive_components {
        center := primitive_get_center(prim_comp^)
...
bvh_system_add_entity :: proc(system: ^Sys_Bvh, entity: ecs.EntityID) {
    system.rebuild = true
...
bvh_system_remove_entity :: proc(system: ^Sys_Bvh, entity: ecs.EntityID) {
    system.rebuild = true
...
bvh_system_get_original_entities :: proc(system: ^Sys_Bvh) -> []ecs.EntityID {
