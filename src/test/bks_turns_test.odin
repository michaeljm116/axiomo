...
setup_level :: proc() {
    // start_level1 will set game.g.level, player, bees, deck, etc.
    game.start_level1(&game.g.level)
...
test_run_players_turn_movement_flow :: proc(t: ^testing.T) {
    ...
    game.run_players_turn(&pt, &gs, &game.g.level.player, &game.g.level.bees, &bee_sel, &bee_near_local)
...
test_run_players_turn_enemy_selection_and_focus :: proc(t: ^testing.T) {
    ...
    testing.expect(t, .PlayerFocused in game.g.level.bees[0].flags, "Bee 0 should have PlayerFocused flag after pressing 'F'")
}
...
test_perform_bee_sting_reduces_player_health :: proc(t: ^testing.T) {
    ...
    game.g.level.player.pos = vec2{0, 0}
    game.g.level.player.health = 5

    // Ensure there's at least one bee
    testing.expect(t, len(game.g.level.bees) > 0, "Level should have at least one bee for this test")

    b := &game.g.level.bees[0]
    b.pos = vec2{0, 1} // adjacent
    b.flags ~= {}      // clear flags

    prev_hp := game.g.level.player.health
    // bee_action_select(.Sting, b, &game.g.level.player)

    testing.expect(t, game.g.level.player.health == prev_hp - 1, "Sting action should reduce player health by 1 when player not dodging")
}
...
test_bee_turn_draws_from_deck_and_performs_action :: proc(t: ^testing.T) {
    ...
    game.g.level.player.pos = vec2{2, 2}
    game.g.level.player.health = 4

    // Ensure we have at least one bee and make it Normal so bee_turn draws 1 card
    testing.expect(t, len(game.g.level.bees) > 0, "Level should have bees for this test")
    b := &game.g.level.bees[0]
    ...
    testing.expect(t, game.g.level.player.health < prev_hp, "bee_turn with Sting drawn should reduce player health")
}
