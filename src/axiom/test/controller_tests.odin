...
set_key_state :: proc(key: i32, pressed: bool, just_pressed: bool = false, just_released: bool = false) {
    ...
    g_input.keys_pressed[key]      = pressed
    g_input.keys_just_pressed[key] = just_pressed
    g_input.keys_just_released[key]= just_released
}
...
setup :: proc(t: ^testing.T) {
    reset_input_state()
    g_controller = {} // zero out controller
    controller_init_default_keyboard(&g_controller)
}
...
test_button_press_hold_release :: proc(t: ^testing.T) {
    ...
    b := &g_controller.buttons[.ActionU]
    ...
    controller_handle_button(b, 0.016)

    expect(t, .JustPressed in b.action)
    expect(t, .Pressed    in b.action)
    expect(t, b.val == 1)
    ...
    expect(t, .JustReleased in b.action)
    expect(t, .Released     in b.action)
    expect(t, b.val == 0)
    expect(t, b.time.curr == 0)
    expect(t, !(.Pressed in b.action))
    expect(t, !(.Held    in b.action))
...
test_just_flags_are_one_frame_only :: proc(t: ^testing.T) {
    ...
    b := &g_controller.buttons[.ActionD]
    b.key = glfw.KEY_E
...
test_dpad_axis_basic_directions :: proc(t: ^testing.T) {
    ...
    controller_handle_keyboard(&g_controller, 0.016)

    expect(t, g_controller.left_axis.x ==  0)
    expect(t, g_controller.left_axis.y == -1)   // note your sign flip
    expect(t, g_controller.left_axis.isMoving)
    ...
    expect(t, g_controller.left_axis.x == 0)
    expect(t, g_controller.left_axis.y == 0)    // still from previous W
    expect(t, !g_controller.left_axis.isMoving)
    ...
    expect(t, g_controller.left_axis.x ==  1)
    expect(t, g_controller.left_axis.y ==  0)
    expect(t, g_controller.left_axis.isMoving)
}
...
test_axis_diagonal_and_zero :: proc(t: ^testing.T) {
    ...
    expect(t, g_controller.left_axis.x ==  1)
    expect(t, g_controller.left_axis.y == -1)
    expect(t, g_controller.left_axis.isMoving)
...
// ─────────────────────────────────────────────
// Deadzone helper (controller_handle_epsilon)
// ─────────────────────────────────────────────
...
test_epsilon_deadzone :: proc(t: ^testing.T) {
    ...
    expect(t, controller_handle_epsilon(0.00)  == 0.0)
    expect(t, controller_handle_epsilon(0.04)  == 0.0)
    expect(t, controller_handle_epsilon(0.049) == 0.0)
    expect(t, controller_handle_epsilon(0.05)  >  0.0)
    expect(t, controller_handle_epsilon(0.051) >  0.0)

    expect(t, controller_handle_epsilon(-0.07) <  0.0)
    expect(t, controller_handle_epsilon(-0.049) == 0.0)

    v := 0.333
    expect(t, controller_handle_epsilon(v) == v)   // > 0.05 → unchanged
}