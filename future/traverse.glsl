...
{
    float t_scale = length(inv_ray.d);
    for (int f = prim.startIndex; f < prim.endIndex; ++f)
    {
        vec4 t_quad = quadIntersect(inv_ray, faces[f]);
        float world_t = t_quad.x * t_scale; //convert local_t to world_t
        if ((world_t > EPSILON) && (world_t < ray.t))
        {
            ray.t = world_t;
            hit.t = world_t;
...
HitInfo traverse(inout Ray ray, in int index, in int offset)
{
    ...
    switch (type) {
        ...
        case TYPE_CONE:
        {
            set_hit_w_normal(hit, ray, coneIntersect(ray, prim));
            break;
        }
        case TYPE_MESH:
        {
            ...
            if (!t_mesh.b) break;
            float t_scale = length(inv_ray.d);
            float world_t = t_mesh.t * t_scale;
            if ((world_t > EPSILON) && (world_t < ray.t)) {
                hit.prim_id = index;
...
float quick_traverse(inout Ray ray, in int offset, in float val)
{
    ...
    switch (type)
    {
        ...
        case TYPE_CONE:
        {
            set_hit_with_normal(hit, coneIntersect(ray, prim));
            if (is_hit(hit.t, ray.t)) return val;
            break;
        }
        case TYPE_MESH:
