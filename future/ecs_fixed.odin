...
// Entity ID packing: lower 48 bits = index, upper 16 bits = version
ENTITY_INDEX_BITS :: u64 = 48
ENTITY_INDEX_MASK :: EntityID = EntityID(((u64(1) << ENTITY_INDEX_BITS) - 1))

pack_entity :: proc(index: u64, version: u16) -> EntityID {
    return EntityID(((u64(version) << ENTITY_INDEX_BITS) | (index & u64(ENTITY_INDEX_MASK))))
}

unpack_entity :: proc(id: EntityID) -> (u64, u16) {
    raw := u64(id)
    index := raw & u64(ENTITY_INDEX_MASK)
    version := u16(raw >> ENTITY_INDEX_BITS)
    return index, version
}

// Return an EntityID suitable as map key (index-only, version zeroed)
base_entity :: proc(id: EntityID) -> EntityID {
    idx, _ := unpack_entity(id)
    return EntityID(idx)
}
// Errors
Error :: enum {
...
add_entity :: proc(world: ^World) -> EntityID {
	raw := world.next_entity_id
	idx, _ := unpack_entity(raw)
	base := EntityID(idx)
	// Advance the next free index (version zeroed)
	world.next_entity_id = pack_entity(idx + 1, 0)

	if info, exists := &world.entity_index[base]; exists {
		info.archetype = nil
		info.row = -1
		info.version += 1
	} else {
		world.entity_index[base] = EntityInfo{
			archetype = nil,
			row = -1,
			version = 0,
		}
	}
	version := u16(world.entity_index[base].version)
	return pack_entity(idx, version)
}
create_entity :: add_entity

remove_entity :: proc(world: ^World, entity: EntityID) {
	info, ok := &world.entity_index[base_entity(entity)]
	...
	if row != last_row {
		...
		if moved_info, exists := &world.entity_index[base_entity(last_entity)]; exists {
			moved_info.row = row
		}
	}
	...
	info.row = -1
	info.version += 1
	world.next_entity_id = base_entity(entity)
...
has_component_type :: proc(world: ^World, entity: EntityID, $T: typeid) -> bool {
	info, exists := world.entity_index[base_entity(entity)]
...
add_component :: proc(world: ^World, entity: EntityID, component: $T) {
    ...
    info := world.entity_index[base_entity(entity)]
    ...
    when size_of(T) > 0 {
        index := world.entity_index[base_entity(entity)].row
...
move_entity :: proc(
	...
	append(&new_archetype.entities, base_entity(entity))

	// Update entity index
	world.entity_index[base_entity(entity)] = EntityInfo {
	...
	if old_archetype != nil && info.row >= 0 {
		...
		if old_row != last_index {
			...
			if entity_info, exists := &world.entity_index[base_entity(last_entity)]; exists {
	...
	if old_archetype != nil && len(old_archetype.entities) == 0 {
		...
		remove_archetype_from_queries(world, old_archetype)
	}
}
...
remove_archetype_from_queries :: proc(world: ^World, archetype: ^Archetype) {
	if archetype == nil { return }
	for i := 0; i < len(world.queries); i += 1 {
		q := &world.queries[i]
		// remove all occurrences of this archetype from q.archetypes
		write_idx := 0
		for _, a in q.archetypes {
			if a != archetype {
				q.archetypes[write_idx] = a
				write_idx += 1
			}
		}
		resize(&q.archetypes, write_idx)
	}
}
