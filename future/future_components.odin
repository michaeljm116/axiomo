// Verify sizes match Jolt
#assert(size_of(ObjectLayer) == size_of(jolt.Object_Layer))  // u16

// Components
Cmp_Physics :: struct {
    system: ^jolt.Physics_System,
    body_interface: ^jolt.Body_Interface,
    activation_listener: ^jolt.Activation_Listener,
    contact_listener: ^jolt.Contact_Listener,
    temp_allocator: ^jolt.Temp_Allocator,
    job_system: ^jolt.Job_System,
    object_layer_filter: ^jolt.Object_Layer_Pair_Filter,
    broad_phase_filter: ^jolt.Object_vs_BroadPhase_Layer_Filter,
}

Cmp_RigidBody :: struct {
    body_id: jolt.BodyID,
...
rigid_body_init :: proc() -> Cmp_RigidBody{
    return Cmp_RigidBody{
        jolt.BodyID_Invalid,
        .Static,
        0.0,
        0.2,
...
Cmp_Collider :: struct {
    ...
    extents: vec3,
    offset: vec3,
    layer: jolt.Object_Layer,
}

collider_init :: proc() -> Cmp_Collider{
    return Cmp_Collider{
        shape = nil,
        type = .Box,
        extents = {1,1,1},
        offset = {0,0,0},
        layer = u16(ObjectLayer.NON_MOVING)
    }
}
...
cmp_collided_destroy :: proc(c: ^Cmp_Collided) {
    delete(c.collisions)
}

// Conversion helpers for Jolt (single and double precision)
to_jolt_vec3 :: proc(v: Vec3) -> jolt.Vec3 {
    return jolt.Vec3{v.x, v.y, v.z}
}

to_jolt_quat :: proc(q: Quat) -> jolt.Quat {
    return jolt.Quat{q.x, q.y, q.z, q.w}
}

from_jolt_vec3 :: proc(v: jolt.Vec3) -> Vec3 {
    return Vec3{v.x, v.y, v.z}
}

from_jolt_quat :: proc(q: jolt.Quat) -> Quat {
    return Quat{q.x, q.y, q.z, q.w}
}

when jolt.DOUBLE_PRECISION {
    to_jolt_rvec3 :: proc(v: Vec3) -> jolt.RVec3 {
        return jolt.RVec3{f64(v.x), f64(v.y), f64(v.z)}
    }
    from_jolt_rvec3 :: proc(v: jolt.RVec3) -> Vec3 {
